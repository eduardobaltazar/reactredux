...
function rootReducer(state = initialState, action) {
  ...
  switch(action.type) {
    ...
    case LOGOUT:
      return Object.assign({}, state, { tasks: { data: action.payload.tasks }, authentication: action.payload.authentication });
    default:
      return state;
  }
...
function addTaskReducer(state, action) {
  ...
  var lastTask = state.tasks.data[state.tasks.data.length - 1];

  task.id = lastTask.id+1;

  return Object.assign({}, state, { tasks: { data: [...state.tasks.data, task] } });
}

function completeTaskReducer(state, action) {
  var id = action.payload.id;

  return Object.assign({}, state, { tasks: { data: state.tasks.data.map(task => {
    if(id === task.id) {
